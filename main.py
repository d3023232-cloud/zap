...
async def msg_new_location_name(message: Message, state: FSMContext):
    data = await state.get_data()
    if 'new_loc_key' not in data:
        return  # Не наша FSM, игнорируем
...
async def admin_take_money_exec(message: Message, state: FSMContext):
    try:
        ...
        await state.clear()
        await message.answer(
            f"✅ Забрано <b>{amount:,.0f} ₽</b> у {user['full_name']}\n"
...
async def admin_give_fuel_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "⛽ <b>Выдача топлива</b>\n\nФормат: <code>ID_станции тип_топлива литры</code>\nПример: <code>1 AI95 10000</code>",
        reply_markup=back_kb()
    )
    await call.answer()
...
async def admin_rep_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "⭐ <b>Изменение репутации</b>\n\nФормат: <code>ID_станции новая_репутация(0-100)</code>",
        reply_markup=back_kb()
    )
    await call.answer()
...
async def admin_find_exec(message: Message, state: FSMContext):
    try:
        ...
        for s in stations:
            ...
            stations_text += f"\n• «{s['name']}» | Реп: {s['reputation']} | {int(total_fuel):,} л"
